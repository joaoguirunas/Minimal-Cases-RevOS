...
interface ExtraFieldsCardProps {
  personId: string;
}

export function ExtraFieldsCard({ personId }: ExtraFieldsCardProps) {
  const [isEditing, setIsEditing] = useState(false);
  const [draft, setDraft] = useState<Record<string, unknown>>({});
  const [saving, setSaving] = useState(false);

  const { data: fieldDefs = [], isLoading: isLoadingDefs } = useLeadFieldDefinitionsByEntity('pessoa');
  const { data: fieldValues = [], isLoading: isLoadingVals } = useLeadFieldValuesByEntity('pessoa', personId);
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          field.type === "textarea" || isEditing ? "space-y-1.5" : "flex items-start justify-between gap-3"
                        )}
                      >
                        <div className="flex items-center gap-1.5 min-w-0 flex-shrink-0">
                          ...
                          {field.agent_managed && (
                            <Bot className="w-2.5 h-2.5 text-primary/40 flex-shrink-0" strokeWidth={1.5} />
                          )}
                        </div>
                        ...
                        {isEditing ? (
                          ...
                          <span className={cn(
                            ...
                            field.type === "textarea" ? "" : "text-right max-w-[60%]",

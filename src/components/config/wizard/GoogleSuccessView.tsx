...
export default function GoogleSuccessView({
  ...
  return (
    <div className="border border-emerald-500/25 rounded-lg bg-card overflow-hidden">
      {/* Header */}
            ...
            <div key={c.user_id} className="px-5 py-3 flex items-center gap-3">
              ...
              {c.status === 'healthy' ? (
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <Badge variant="outline" className="gap-1 rounded-md text-[10px] text-red-600 border-red-500/30 bg-red-500/5">
                    <XCircle className="w-3 h-3" aria-hidden="true" /> Token expirado
                  </Badge>
                  <Button
                    size="sm"
                    variant="outline"
                    className="h-[26px] text-[11px] rounded-md px-2"
      ...
      <div className="px-5 py-3 border-t border-border flex flex-wrap items-center gap-2">
        ...
        <Button
          size="sm"
          variant="ghost"
          className="h-[30px] text-xs rounded-lg gap-1.5 text-muted-foreground"
        ...
        <AlertDialog>
          <AlertDialogTrigger asChild>
            <Button
              size="sm"
              variant="ghost"
              className="h-[30px] text-xs rounded-lg gap-1.5 text-red-600 hover:text-red-600 hover:bg-red-500/10 ml-auto"
            >

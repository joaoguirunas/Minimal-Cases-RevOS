-- Popular dados para a tenant "Receita Previsível" de forma segura
...
BEGIN
    ...
    -- 1. Verificar se já existe pipeline, senão criar
    ...
    -- 2. Criar motivos de perda se não existem
    IF NOT EXISTS (SELECT 1 FROM crm_motivo_perda WHERE tenant_id = tenant_uuid) THEN
    ...
    -- 3. Criar empresas se não existem muitas
    IF (SELECT COUNT(*) FROM crm_empresas WHERE tenant_id = tenant_uuid) < 5 THEN
        ...
        ) AS empresas(nome_fantasia, razao_social, cnpj, telefone, email, site, tenant_id, status)
        WHERE NOT EXISTS (
            SELECT 1 FROM crm_empresas e2 
            WHERE e2.nome_fantasia = empresas.nome_fantasia AND e2.tenant_id = empresas.tenant_id
        );
    END IF;

    -- 4. Criar pessoas se não existem muitas  
    IF (SELECT COUNT(*) FROM crm_pessoas WHERE tenant_id = tenant_uuid) < 5 THEN
        ...
        ) AS pessoas(nome, email, whatsapp, documento, origem, score, tenant_id, status, objetivo, momento, renda)
        WHERE NOT EXISTS (
            SELECT 1 FROM crm_pessoas p2 
            WHERE p2.email = pessoas.email AND p2.tenant_id = pessoas.tenant_id
        );
    END IF;

    -- 5. Criar times se não existem muitos
    IF (SELECT COUNT(*) FROM crm_times WHERE tenant_id = tenant_uuid) < 5 THEN
        ...
        ) AS times(nome, descricao, tipo, prioridade, tenant_id, ativo)
        WHERE NOT EXISTS (
            SELECT 1 FROM crm_times t2 
            WHERE t2.nome = times.nome AND t2.tenant_id = times.tenant_id
        );
    END IF;

    -- 6. Criar negócios se não existem muitos e há pessoas/empresas suficientes
    IF (SELECT COUNT(*) FROM crm_leads WHERE tenant_id = tenant_uuid AND pipeline_id = pipeline_uuid) < 3 AND
       (SELECT COUNT(*) FROM crm_pessoas WHERE tenant_id = tenant_uuid) >= 3 AND
       (SELECT COUNT(*) FROM crm_empresas WHERE tenant_id = tenant_uuid) >= 3 THEN
       
        -- Inserir alguns negócios de exemplo usando dados existentes
        WITH 
            pessoas_sample AS (SELECT id FROM crm_pessoas WHERE tenant_id = tenant_uuid ORDER BY created_at LIMIT 3),
            empresas_sample AS (SELECT id FROM crm_empresas WHERE tenant_id = tenant_uuid ORDER BY created_at LIMIT 3),
            stages_sample AS (SELECT id, ordem FROM crm_stages WHERE pipeline_id = pipeline_uuid ORDER BY ordem LIMIT 3)
        INSERT INTO crm_leads (titulo, person_id, empresa_id, pipeline_id, stage_id, valor, status, tenant_id, utm_source, utm_medium)
        SELECT 
            'Negócio ' || s.ordem || ' - ' || 'Projeto Consultoria',
            p.id,
            e.id,
            pipeline_uuid,
            s.id,
            (15000 + s.ordem * 5000)::numeric,
            'em-andamento',
            tenant_uuid,
            'google',
            'organic'
        FROM 
            ...
            stages_sample s
        WHERE 
            p.id = (SELECT id FROM pessoas_sample ORDER BY id OFFSET (s.ordem - 1) LIMIT 1) AND
            e.id = (SELECT id FROM empresas_sample ORDER BY id OFFSET (s.ordem - 1) LIMIT 1);
    END IF;

    RAISE NOTICE 'Dados de exemplo populados para tenant: % (pipeline: %)', tenant_uuid, pipeline_uuid;
END $$;
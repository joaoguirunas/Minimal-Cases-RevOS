...
-- Política para permitir que apenas gestores e admins deletem pessoas
...
USING (
  EXISTS (
    ...
    AND tenant_id = crm_pessoas.tenant_id 
    AND (gestor = true OR super_adm = true)
  )
);

-- Política para permitir que apenas gestores e admins deletem negócios
DROP POLICY IF EXISTS "Users can delete leads from their tenant" ON crm_leads;
...
USING (
  EXISTS (
    SELECT 1 FROM crm_usuarios 
    WHERE auth_user_id = auth.uid() 
    AND tenant_id = crm_leads.tenant_id 
    AND (gestor = true OR super_adm = true)
  )
);

-- Política para permitir que apenas gestores e admins deletem agendamentos
DROP POLICY IF EXISTS "Users can delete agendamentos in their tenant" ON crm_agendamentos;
...
-- Função para deletar pessoa e dados relacionados

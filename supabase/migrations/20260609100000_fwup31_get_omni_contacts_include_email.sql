-- fwup31: get_omni_contacts v3 — include email-only contacts (FORM PRO leads)
--
-- Change vs fwup30:
--   Canal filter now also matches `email IS NOT NULL`. Contacts created via
--   lp-submit from email-only forms (ebooks, webinars, lead magnets) were
--   stored in clients_people and received `messages` rows with
--   source_type='form', but the Omni inbox query (this RPC + the direct
--   fallback in useConversasSimples.ts) filtered them out — making the
--   conversation invisible to operators forever.
...
AS $$
  ...
  WHERE
    -- canal: whatsapp, instagram, tiktok, or email obrigatório
    -- email included for FORM PRO leads that have no messaging channel yet
    (

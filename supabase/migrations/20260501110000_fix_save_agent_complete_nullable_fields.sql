-- Fix save_agent_complete: allow voice_id and voice_model_id to be explicitly set to NULL.
--
-- Bug: voice_id and voice_model_id use COALESCE, which treats JSON null as SQL NULL
-- and falls back to the old value. This means selecting "Padrão" (which maps to null)
-- never clears the field in the database — the old value persists.
--
-- Fix: convert to CASE WHEN p_agent_data ? 'key' THEN value ELSE old END pattern,
-- matching the same approach used for UUID fields (pipeline_id, leads_stages_id, etc.).
--
-- Also fixes: voice_first_message (nullable text), wa_phone_number_id (nullable text),
-- and llm_model / llm_provider (nullable after provider reset via llm_model: undefined).

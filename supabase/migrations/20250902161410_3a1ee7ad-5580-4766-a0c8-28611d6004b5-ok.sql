...
-- Policy for SELECT: Users can view their own profile + rafaela can view all
...
-- Policy for INSERT: Only rafaela and super admins can create users
...
-- Policy for SELECT: Users can see their tenant + super admins see all + rafaela sees all
...
-- Create new function to get available tenants for current user (with correct signature)
...
AS $$
  ...
  UNION ALL
  -- Special case for rafaela@iatize.com - show all tenants
  SELECT 
    t.id,
    t.name,
    t.value,

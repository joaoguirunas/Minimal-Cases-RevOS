-- ╔════════════════════════════════════════════════════════════════════════════╗
-- ║  Apply Prompt v3 to production diagnostic agent                         ║
-- ║  Agent: "João Guirunas — Diagnóstico" (d0c29089-...)                   ║
-- ║  Previous migration found agent via ILIKE but Unicode ó broke match     ║
-- ╚════════════════════════════════════════════════════════════════════════════╝
...
BEGIN
  -- Verify agent exists
  SELECT EXISTS(SELECT 1 FROM public.ai_agents WHERE id = v_agent_id) INTO v_exists;

  IF NOT v_exists THEN
    RAISE NOTICE '[Prompt-v3] Agent % not found — aborting.', v_agent_id;
    RETURN;
  END IF;

  RAISE NOTICE '[Prompt-v3] Updating agent "João Guirunas — Diagnóstico" to v3: %', v_agent_id;
  ...
  INSERT INTO public.ai_agents_history (
    ...
    jsonb_build_object(
      ...
      'detalhes', jsonb_build_object(
        ...
        'temperatura', 'pre_sale_temperature (1-5) adicionado como input + tool atualizar_lead',
        'probabilidade', 'close_probability (1-5) adicionado como input + tool atualizar_lead',
  ...
  RAISE NOTICE '[Prompt-v3] Agent updated successfully.';
END;
$migration$;

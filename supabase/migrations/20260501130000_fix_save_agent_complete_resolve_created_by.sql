-- Fix: ai_agents_history.created_by FK violation (Postgres 23503).
--
-- Root cause:
--   ai_agents_history.created_by → settings_users(id) (FK).
--   Frontend (src/hooks/useAgentesIAReal.ts) sends p_created_by = auth.users.id
--   from supabase.auth.getUser(), but the FK expects settings_users.id.
--   These are distinct entities — settings_users has its own surrogate id and
--   only references auth.users via auth_user_id.
--
-- Fix strategy (Opção A):
--   Make the RPC defensive: resolve the canonical settings_users.id internally.
--   Accept either form on input — auth.users.id (current behavior) or
--   settings_users.id (future-proof) — and fall back to auth.uid() when nothing
--   was provided. If no settings_users row maps to the caller, store NULL
--   (FK is ON DELETE SET NULL, so NULL is allowed).
--
-- Only the INSERT into ai_agents_history changes versus the previous version.
-- Everything else is preserved verbatim from 20260501120000_fix_save_agent_complete_array_types.sql.
...
BEGIN
  ...
  -- Resolve created_by to a valid settings_users.id, regardless of what the
  -- caller supplied. Try in order:
  --   1. p_created_by already IS a settings_users.id   → use as-is
  --   2. p_created_by is an auth.users.id              → look up via auth_user_id
  --   3. fallback: resolve from auth.uid()             → look up via auth_user_id
  -- If nothing matches, leave NULL (FK allows NULL via ON DELETE SET NULL).
  ...
  VALUES
    (p_agent_id, v_current_version, v_snapshot, p_changelog, v_resolved_user_id);

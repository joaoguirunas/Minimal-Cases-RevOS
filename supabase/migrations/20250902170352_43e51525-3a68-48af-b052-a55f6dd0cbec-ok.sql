...
-- PARTE 1: DROPAR POLÍTICAS DA TABELA crm_tenants QUE TAMBÉM DEPENDEM DAS FUNÇÕES
-- ========================================================================

-- Dropar políticas da tabela crm_tenants que usam as funções problemáticas
DROP POLICY IF EXISTS "tenants_safe_select" ON crm_tenants;
DROP POLICY IF EXISTS "tenants_safe_update" ON crm_tenants;
DROP POLICY IF EXISTS "tenants_safe_insert" ON crm_tenants;
DROP POLICY IF EXISTS "tenants_safe_delete" ON crm_tenants;

-- Agora dropar todas as políticas que usam user_has_tenant_access

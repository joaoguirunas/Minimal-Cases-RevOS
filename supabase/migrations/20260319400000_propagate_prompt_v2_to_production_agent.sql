-- ╔════════════════════════════════════════════════════════════════════════════╗
-- ║  Propagate Prompt v2 to PRODUCTION Diagnóstico agent                    ║
-- ║                                                                          ║
-- ║  Root cause: migration 20260319300000 only updated the TEMPLATE          ║
-- ║  (is_template=true), but the production agent is a CLONE (is_template=   ║
-- ║  false) with its own identity/rules/steps. This migration:               ║
-- ║    1. Updates the production Diagnóstico agent with v2 prompt            ║
-- ║    2. Backfills clients_people.goal/moment from score_category_items     ║
-- ╚════════════════════════════════════════════════════════════════════════════╝

-- ══════════════════════════════════════════════════════════════════════════════
-- SECTION 1 — Update production Diagnóstico agent with v2 prompt
-- ══════════════════════════════════════════════════════════════════════════════
...
BEGIN
  -- Find the PRODUCTION agent (NOT template)
  ...
  IF v_agent_id IS NULL THEN
    RAISE NOTICE '[Prompt-v2-prod] Production Diagnóstico agent not found — pulando.';
  ELSE
    RAISE NOTICE '[Prompt-v2-prod] Updating PRODUCTION Diagnóstico agent: %', v_agent_id;
    ...
    INSERT INTO public.ai_agents_history (
      ...
      jsonb_build_object(
        'resumo', 'Propagate v2 prompt to PRODUCTION agent (was only on template)',
        'fix', 'Previous migration 20260319300000 only updated is_template=true, production agent was still on v1',
        'timestamp', NOW()
      ),
      NOW()
    );

    RAISE NOTICE '[Prompt-v2-prod] Production Diagnóstico agent updated successfully.';
  END IF;
END;
$sec1$;

-- ══════════════════════════════════════════════════════════════════════════════
-- SECTION 2 — Backfill clients_people.goal/moment from score_category_items
...
BEGIN
  ...
  LOOP
    ...
    -- Get category_selections from score_matrix
    ...
    IF v_selections IS NOT NULL THEN
      ...
      IF v_item_ids IS NOT NULL AND array_length(v_item_ids, 1) > 0 THEN
        -- Resolve item names with their category slugs
        FOR v_item IN

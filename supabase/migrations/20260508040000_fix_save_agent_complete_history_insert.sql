-- ═══════════════════════════════════════════════════════════════════
-- 20260508040000_fix_save_agent_complete_history_insert.sql
--
-- Two bugs coexisted in save_agent_complete:
--   1. stage_ids CASE produced text[] vs uuid[] column → 42846 (fixed in 030000)
--   2. INSERT INTO ai_agents_history omitted `version` and `data` (both NOT NULL)
--      → 23502 null value violates not-null constraint
--
-- Root cause of #2: incremental patches removed the original SELECT FOR UPDATE
-- that reads current_version + snapshot, and the correct INSERT pattern.
--
-- Fix: restore the lock+snapshot read, correct history INSERT, and
-- current_version increment — keeping all CASE WHEN type fixes from 030000.
-- ═══════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  VALUES (p_agent_id, v_current_version, v_snapshot, COALESCE(p_changelog, '{}'::jsonb), v_created_by);

...
-- 1. Obter o tenant_id da "Receita Previsível"
DO $$ 
DECLARE 
    tenant_uuid UUID;
    pipeline_uuid UUID;
...
BEGIN
    ...
    -- 8. Criar mensagens/conversas
    INSERT INTO crm_messages (lead_id, pessoa_id, from_message, message, canal, tipo_mensagem, tenant_id) VALUES
    (lead_ids[1], pessoa_ids[1], pessoa_ids[1]::text, 'Olá! Gostaria de saber mais sobre consultoria em vendas para nossa empresa.', 'whatsapp', 'texto', tenant_uuid),
    ...
    (lead_ids[2], pessoa_ids[2], pessoa_ids[2]::text, 'Preciso implementar um CRM na nossa empresa. Vocês podem ajudar?', 'whatsapp', 'texto', tenant_uuid),
    ...
    (lead_ids[3], pessoa_ids[3], pessoa_ids[3]::text, 'Nossa equipe de vendas precisa de treinamento. Vocês oferecem isso?', 'whatsapp', 'texto', tenant_uuid);
    ...
    RAISE NOTICE 'Dados de exemplo criados com sucesso para tenant: %', tenant_uuid;
END $$;
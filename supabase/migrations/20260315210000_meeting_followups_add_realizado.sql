-- ══════════════════════════════════════════════════════════════════════════════
-- Add 'realizado' status to meeting follow-ups system
-- Also unblock 'cancelado' follow-ups in the DB trigger
-- ══════════════════════════════════════════════════════════════════════════════

-- ─── 1. Expand CHECK constraint on meetings_followups ───────────────────────
...
ALTER TABLE public.meetings_followups
  ...
  CHECK (meeting_status IN ('agendado', 'compareceu', 'nao_compareceu', 'cancelado', 'realizado'));

-- ─── 2. Update DB trigger to handle 'realizado' + unblock 'cancelado' ───────
